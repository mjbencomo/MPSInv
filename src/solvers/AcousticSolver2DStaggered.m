classdef AcousticSolver2DStaggered < AcousticSolver
    % AcousticSolver2DStaggered solves the 2D first-order acoustic system.
    % Pressure is stored on a primal-primal grid. The x-velocity is stored
    % on a dual-primal grid and the y-velocity on a primal-dual grid.
    % SpatialOrder selects second- or fourth-order centered staggered
    % differences. Leapfrog time stepping is second order.
    ...
    methods
        function obj = AcousticSolver2DStaggered( ...
            ...
            if ~isa(computationalGrid.x,'Grid1DUnifPrimal') || ...
                ...
                error('AcousticSolver2DStaggered:PrimalGridRequired', ...
                    ['Computational pressure requires uniform primal ' ...
                     'grids in both coordinate directions.']);
            end

            if options.SpatialOrder == 4 && ...
                    any(computationalGrid.N < 5)
                error('AcousticSolver2DStaggered:GridTooSmall', ...
                    ['At least five pressure grid points in each direction ' ...
                     'are required for the fourth-order stencil.']);
            end

            if any(outputGrid.dom(:,1) < computationalGrid.dom(:,1)) || ...
                    any(outputGrid.dom(:,2) > computationalGrid.dom(:,2))
                error('AcousticSolver2DStaggered:OutputGridOutsideDomain', ...
            ...
            [obj.K,obj.Bx,obj.By,~,obj.cMax] = sampleMedium2D( ...
                computationalGrid,medium,BetaGrid="staggered");
            obj.cfl = obj.cMax*obj.dt*sqrt(sum(1./computationalGrid.h.^2));
            ...
            if options.EnforceCFL && obj.cfl > cflLimit+100*eps
                error('AcousticSolver2DStaggered:CFLViolation', ...
                    ['The multidimensional CFL number ' ...
                     'cMax*dt*sqrt(dx^(-2)+dy^(-2)) = %.6g exceeds the ' ...
                     'order-%d stability limit %.6g.'], ...
                    obj.cfl,obj.spatialOrder,cflLimit);
            end

            if obj.velocityExtrapolation == "none"
                obj.validateVelocityOutputDomain(obj.Bx.grid,"x");
                obj.validateVelocityOutputDomain(obj.By.grid,"y");
            end
        end

        function [pressure,velocityX,velocityY] = solve(obj,p0,vx0,vy0,options)
            % Solve and sample pressure and both velocity components.
            ...
            velocityX = GridFunctionTimeSeries( ...
                obj.outputGrid,obj.outputTimes, ...
                Label="x-particle velocity",Units="m/s",TimeUnits="s");
            velocityY = GridFunctionTimeSeries( ...
                obj.outputGrid,obj.outputTimes, ...
                Label="y-particle velocity",Units="m/s",TimeUnits="s");

            pressureGrid = obj.computationalGrid;
            velocityXGrid = obj.Bx.grid;
            velocityYGrid = obj.By.grid;
            dx = pressureGrid.h(1);
            dy = pressureGrid.h(2);
            dt = obj.dt;
            times = obj.computationalTimes;

            pressureSource = options.Source.preparePressure(pressureGrid);
            velocityXSource = options.Source.prepareVelocityX(velocityXGrid);
            velocityYSource = options.Source.prepareVelocityY(velocityYGrid);

            p = obj.dataOnGrid(p0,pressureGrid,'pressure');
            vx0Values = obj.dataOnGrid(vx0,velocityXGrid,'x-velocity');
            vy0Values = obj.dataOnGrid(vy0,velocityYGrid,'y-velocity');
            p = obj.applyPressureBoundary(p);
            ...
            fx = options.Source.evaluatePrepared(velocityXSource,times(1));
            fy = options.Source.evaluatePrepared(velocityYSource,times(1));
            vxHalf = vx0Values-(dt/2)*obj.Bx.values.*DxP+(dt/2)*fx;
            vyHalf = vy0Values-(dt/2)*obj.By.values.*DyP+(dt/2)*fy;
            ...
            for n = 2:obj.Nt
                ...
                divergence = DxVx(:,2:end-1)+DyVy(2:end-1,:);
                tHalf = 0.5*(times(n-1)+times(n));
                fp = options.Source.evaluatePrepared(pressureSource,tHalf);

                p(2:end-1,2:end-1) = pOld(2:end-1,2:end-1) ...
                    -dt*obj.K.values(2:end-1,2:end-1).*divergence ...
                    +dt*fp(2:end-1,2:end-1);
                p = obj.applyPressureBoundary(p);
                ...
                DxP = obj.derivativePrimalToDualX(p,dx);
                DyP = obj.derivativePrimalToDualY(p,dy);
                fx = options.Source.evaluatePrepared(velocityXSource,times(n));
                fy = options.Source.evaluatePrepared(velocityYSource,times(n));
                vxHalf = vxOld-dt*obj.Bx.values.*DxP+dt*fx;
                vyHalf = vyOld-dt*obj.By.values.*DyP+dt*fy;
            ...
            if pOutputIndex <= obj.NtOutput || ...
                ...
                error('AcousticSolver2DStaggered:UnfilledOutput', ...
    ...
    methods (Access = private)
        ...
        function p = applyPressureBoundary(~,p)
            p([1,end],:) = 0;
            p(:,[1,end]) = 0;
        end
        ...
        function nextIndex = storeBracketedOutputs(obj,series,nextIndex, ...
            ...
            while nextIndex <= obj.NtOutput && ...
                ...
                if outputTime < leftTime-obj.timeTolerance()
                    error('AcousticSolver2DStaggered:OutputOrdering', ...
        ...
        function values = interpolateSpace(obj,data,sourceGrid,isVelocity)
            ...
            if any(~isfinite(values),'all')
                error('AcousticSolver2DStaggered:SpatialInterpolationFailed', ...
        ...
        function values = dataOnGrid(~,data,grid,fieldName)
            ...
            if isa(data,'GridFunction')
                if data.dim ~= 2
                    error('AcousticSolver2DStaggered:DimensionMismatch', ...
                ...
                try
                    ...
                    error('AcousticSolver2DStaggered:InvalidInitialData', ...
        ...
        function validateVelocityOutputDomain(obj,velocityGrid,component)
            if any(obj.outputGrid.dom(:,1) < velocityGrid.dom(:,1)) || ...
                ...
                error('AcousticSolver2DStaggered:VelocityExtrapolationRequired', ...
                    ['The output grid extends beyond the computational ' ...
                     '%s grid. Use VelocityExtrapolation="linear" or ' ...
                     'choose interior output points.'],component);
            end
        end
